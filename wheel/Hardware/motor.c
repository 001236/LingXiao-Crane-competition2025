...
void motor_pid_control(motor_data *motor_){//依据位置环提供当前误差分析，调节速度环的目标值，速度环负责输出
	counter_now=__HAL_TIM_GetCounter(&motor_->TIM_EncoderHandle);
//	counter_posi=(counter_now-32768.0)/(459*4);
	distanceRef=distance/10/8.3/3.14159265358;
	double ref=80/8.3/3.14159265358,reff=200/8.3/3.14159265358;
	motor_->motor_pid_p.fdb=distanceRef;
	PID_Pos_Calc(&motor_->motor_pid_p);
	int is_posi=(motor_->motor_pid_p.cur_error>0?1:-1);
...
//	}
  if((motor_->motor_pid_p.cur_error>=ref)||(motor_->motor_pid_p.cur_error<=-ref)){
		motor_->motor_pid_v.ref=(motor_->motor_pid_p.cur_error>0?1:-1)*400;
	}
	else if(motor_->motor_pid_p.cur_error<=ref||motor_->motor_pid_p.cur_error>=-ref){
		motor_->motor_pid_v.ref=motor_->motor_pid_p.cur_error/ref*370+(motor_->motor_pid_p.cur_error>0?1:-1)*0.2*50;
	}
	if((motor_->motor_pid_p.cur_error)<=(0.8/8.3/3.14159265358)&&motor_->motor_pid_p.cur_error>=(-0.8/8.3/3.14159265358)){
		motor_->motor_pid_v.ref=0;
		motor_stop(motor_);
